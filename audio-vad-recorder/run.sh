...
export WHISPER_ENABLED=$(jq -r '.whisper_enabled // false' "$CONFIG")
export WHISPER_API_URL=$(jq -r '.whisper_api_url // ""' "$CONFIG")
export WHISPER_API_KEY=$(jq -r '.whisper_api_key // ""' "$CONFIG")
export WHISPER_MODEL=$(jq -r '.whisper_model // "whisper-large-v3"' "$CONFIG")
