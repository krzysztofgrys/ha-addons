...
export LOCAL_INPUT_DIR="$(jq -r '.local_input_dir // ""' "$CONFIG")"
...
IS_LOCAL_MODE=false
[ -n "$LOCAL_INPUT_DIR" ] && [ "$LOCAL_INPUT_DIR" != "null" ] && IS_LOCAL_MODE=true
...
[ "$IS_LOCAL_MODE" = "true" ] && DISPLAY_MODE="LOCAL (${LOCAL_INPUT_DIR})"

echo "[INFO] Starting UniFi Protect Historical Transcriber"
echo "[INFO]   Mode           : ${DISPLAY_MODE}"
